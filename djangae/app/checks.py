...
check_session_csrf_enabled.messages = {
    "SESSION_CSRF_DISABLED": (
        "Please add 'session_csrf.CsrfMiddleware' to MIDDLEWARE_CLASSES"
    )
}
...
def check_csp_sources_not_unsafe():
    ...
    for csp_src_name in CSP_SOURCE_NAMES:
        ...
        if (
            "'unsafe-inline'" in csp_src_values or
            "'unsafe-eval'" in csp_src_values
        ):
            messages.append(csp_src_name + "_UNSAFE")
    return messages

check_csp_sources_not_unsafe.messages = {
    src + "_UNSAFE": "Please remove 'unsafe-inline'/'unsafe-eval' from " + src
    for src in CSP_SOURCE_NAMES
}
